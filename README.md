# IIS configuration to block public site's referrer spam

Crossbill.Plugins.AntiSpam.IIS is the plugin which configures IIS web server to block spamers who target public websites powered by Crossbill products (including Crossbill Cone, Crossbill Bone and Crossbill Nest). The configuration adds the specific rules to IIS for HTTP_REFERER header checks and blacklists the known spam domains. Installing the plugin on Cone, Bone or Nest enhance spam protection of the public website.

## Release version

The compiled version of the plugin can be obtained from [plugin's page on Crossbill Marketplace](https://marketplace.crossbillsoftware.com/en/Apps/Details/Crossbill.Plugins.AntiSpam.IIS/) either by manually downloading a compiled package or by automatically installing it using Crossbill Central.
...
3. Enter the plugin name Crossbill.Plugins.AntiSpam.IIS in the search box (right panel) and click Search;
...
```
{
    ...
    "apps": [
		{
            "Name": "Antispam Plugin for IIS",
            "Environment": "@(Environment)",
            "CentralAgentURL": "@(CentralAgentURL)",
            "AppType": "Nest",
            "NestAppID": "Crossbill.Plugins.AntiSpam.IIS", 
...
The Crossbill Software License Agreement is located in [plugins/Crossbill.Plugins.AntiSpam.IIS/LICENSE.txt](plugins/Crossbill.Plugins.DataVersioning.Git/LICENSE.txt) file.

The Third Party Code in Crossbill Products notice is located in [plugins/Crossbill.Plugins.AntiSpam.IIS/third-party-code.txt](plugins/Crossbill.Plugins.DataVersioning.Git/third-party-code.txt) file.

The copyright and license texts for the third party code can be found in [plugins/Crossbill.Plugins.AntiSpam.IIS/third-party-notices.txt](plugins/Crossbill.Plugins.DataVersioning.Git/third-party-notices.txt) file.
